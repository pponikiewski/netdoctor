...
- 2026-09-25: raport jako PDF przez `printpdf` 0.12 bez funkcji domyślnych (bez HTML, obrazów i silnika układu Azul). Raport nadal powstaje jako tekst, a `src/pdf.rs` rozpoznaje w nim tytuł, nagłówki, wiersze tabel (wcięte, z podwójną spacją: czcionka o stałej szerokości) i zwykłe zdania; treść i jej testy nie zależą od PDF. Czcionki z folderu Windows (Verdana, Lucida Console, zapas Segoe UI/Arial/Consolas/Courier New), osadzane w całości, bo podzbiór w printpdf wymaga funkcji `text_layout` z całym Azulem; bez czcionek zapis wraca do `.txt`. Raport ma datę w nazwie i nie nadpisuje poprzedniego, a domyślny folder to Dokumenty\NetDoctor zamiast AppData. Rodzaj łącza jest pytany, nie wykrywany: antena radiowa i modem LTE są dla komputera zwykłym Ethernetem, a router przez UPnP zwykle też zgłasza Ethernet.

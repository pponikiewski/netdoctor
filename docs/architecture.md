...
- 2026-09-20 — weryfikacja pobranego pliku jest kształtowa, nie kryptograficzna: nagłówek `MZ`, minimum 1 MiB i zgodność rozmiaru z tym, co podaje API. Oparte na zaufaniu do HTTPS i GitHuba, oznaczone `ponytail:` w `update.rs`. Podpisywanie wydań to właściwa odpowiedź, jeśli binarka ma trafiać dalej niż do osób, które wiedzą skąd ją mają.
